...
namespace ORNL
{
    ...
    void PreferencesManager::setDefaultVisualizationColors(std::unordered_map<std::string, std::string> visualizationColorsHex)
    {
        ...
        for(int i = 0; i < visualizationColorsLength; ++i) {
            VisualizationColors colorEnum = (VisualizationColors)i;
            m_visualization_qcolors[VisualizationColorsName(colorEnum).toStdString()] = VisualizationColorsDefaults(colorEnum);
        }
    ...
    QString PreferencesManager::getRotationUnitText()
    {
        ...
        else if (m_rotation_unit == RotationUnit::kXYZ)
    ...
    void PreferencesManager::setTCPServerPort(int port)
    {
        m_tcp_port = port;
    }

    void PreferencesManager::setTcpServerAutoStart(bool start)
    {
        m_tcp_server_autostart = start;
    }

    bool PreferencesManager::getStepConnectivity(StatusUpdateStepType type)
    {
        return m_step_connectivity[(int)type];
    }

    int PreferencesManager::getTCPServerPort()
    {
        return m_tcp_port;
    }

    bool PreferencesManager::getTcpServerAutoStart()
    {
